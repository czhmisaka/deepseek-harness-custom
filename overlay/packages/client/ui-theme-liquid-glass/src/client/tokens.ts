...
export const GLASS_TOKENS: Record<string, string> = {
  ...
  // Markdown code surfaces become dark glass panes; the banner strip stays a
  // lighter scrim, preserving the base theme's lighter-banner relationship
  // (a translucent sticky banner needs the code body to stay darker for
  // occlusion while scrolled code passes beneath).
  '--dsw-alias-markdown-code-block': 'rgba(10, 13, 22, 0.52)',
  '--dsw-alias-markdown-code-block-banner': 'rgba(255, 255, 255, 0.06)',
  // Conversation surfaces: user bubbles and the composer card become thin
  // glass panes. Shared with the goal-command echo bubble and the approval /
  // question / plan-review chat cards (all resolve these two tokens).
  '--dsw-specific-bubble': 'rgba(30, 36, 52, 0.55)',
  '--dsw-specific-input-major': 'rgba(14, 18, 30, 0.55)',
