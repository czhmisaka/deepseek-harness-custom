...
请求用 `provider: deepseek-official` 选择路由；模型 id 原样传到协议，因此新增 DeepSeek 模型无需重新注册。省略 `models` 时会公布 `deepseek-flash`（V4.1 Flash，原生多模态旗舰，适合专注任务、快速且经济）与适合复杂或质量关键任务、能力更强且成本更高的 `deepseek-v4-pro`；每个模型都有 1,000,000 token 上下文窗口。已退役的 `deepseek-v4-flash` 与 `deepseek-v4-flash-vision-exp` id 仍可通过公开 API 的兼容路由调用。显式列表会替换这些默认值，未列出的模型 id 仍作为纯文本路由原样通过。包括模型发现工具在内的客户端可通过 `ctx.llm.listModels('deepseek-official')` 读取这些建议性条目。支持图片的条目可把 `imagePixelBudget` 设置为正整数或 `low`，也可以设置 `imageMaxBytes`。
