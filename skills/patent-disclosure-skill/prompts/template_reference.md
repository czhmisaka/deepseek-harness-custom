...
### 📌「逐条对比最近似专利」模板（写作规范 10，仅中移 13 章分支使用）

**出处：** CMII2203007 P17、HZYF2112012 P13、XX2209082 P14
...
专利 CNxxxxxx 是和本提案最近的。较 CNxxxxxx 本提案，[对比维度 1] 上，本提案[具体改进 1]使[具体效果 1]；[对比维度 2] 上，本提案[具体改进 2]使[具体效果 2]；...

1、与专利 CNxxxxxxA 对比：本提案针对其[缺点描述]，提出[本提案改进思路]，取得[可量化/可定性的效果]。

2、与专利 CNxxxxxxB 对比：本提案针对其[缺点描述]，提出[本提案改进思路]，取得[可量化/可定性的效果]。

3、与专利 CNxxxxxxC 对比：...
```

**要点**：
- **明确点名**第三部分（现有技术）中离得最近的几篇专利（按关联性降序排列）
- **三段式**：该专利的缺点 → 本提案如何改进 → 取得什么技术效果
- **不是泛泛比较**，要在每个专利后面明确点出"缺点 + 改进 + 效果"
- **效果可量化**：能给出数字、置信度、性能提升比例等

**反例**（只是发杂素的"较现有技术性能更优")：**不可**。要具体到："较 CN110874409A 提到的方法，本提案在 1 级濒危/2 级危重样本上预测准确率提升 X%，在心率异常场景下召回率提升 Y%"。
...
### 📌「关键点种字结构」模板（写作规范 8+9，仅中移 13 章分支使用）

**出处：** CMII2203007 P15-16（**6 条典范**）、HZYF2112012 P12-13

**结构骨架**（"种字类型" = 方法/装置/系统/模型/策略/方案/架构）：
...
| 类型词 | 适用场景 | 示例句 |
|--------|---------|--------|
| 方法 | 有明确步骤、顺序的创新 | "一种基于 XYZ 的 XX 处理方法" |
| 装置 | 硬件/软硬结合 | "一种包括 A、B、C 模块的 XX 装置" |
| 系统 | 多模块联合、平台级 | "一种包括...的 XX 系统" |
| 模型 | 机器学习/深度学习/启发式 | "一种 XX 多任务联合训练模型" |
| 策略 | 优化算法、调度 | "一种基于 XX 的调度策略" |
| 方案 | 跨技术点的集成 | "一种面向 XX 场景的端到端方案" |
| 架构 | 框架/层级/模块依赖 | "一种 XX 四层处理架构" |

**要点**：
- **主要发明点**顶在前面（1-2 条）
- **次要发明点**后面展开（3+ 条）
- 每条句式："1、一种 ... XXX **类型**...
- 避免"提供的" "实现的"等动词词重复（可使用"含括"、"包括"、"具有"、"包含"、"采用"）
