...
### 8.1.1 13 条写作规范对照（中移 13 章分支专用）

老大 2026-07-14 从中国移动集团 5 篇范文提炼：

- [ ] **规范 1**：每章段首是否铺场景（行业背景/业务痛点/技术现状）？
- [ ] **规范 2**：现有技术每条是否包含"专利号 + 名称 + 技术方案 + 优劣"四要素？
- [ ] **规范 3**：第四部分缺点是否"逐一对应"第三部分列举的现有专利（句式"针对/对于/针对于 申请号为 X 的专利"）？
- [ ] **规范 4**：本提案思路段是否在第四部分末 + 第五部分开头各出现一次？
- [ ] **规范 5**：技术方案是否按"1 概述 → 2 逻辑 → 2.x 子模块 → 步骤 1-N"三层结构？
- [ ] **规范 6**：每步骤是否给具体业务实施例（"如因 XXX 业务场景..."）？
- [ ] **规范 7**：公式每个变量是否在符号表中解释"含义 + 下标 + 量纲"？
- [ ] **规范 8**：关键点是否用"种字结构"（方法/装置/系统/模型/策略/方案/架构）？
- [ ] **规范 9**：关键点是否按重要性递进，是否标"主要/次要"？
- [ ] **规范 10**：第七部分是否"点名 CN 号 + 逐条对比最近似专利"（三段式：缺点→改进→效果）？
- [ ] **规范 11**：零章术语表是否仅列专用缩写？（不列通用术语）
- [ ] **规范 12**：步骤编号是否稳定可回指（步骤 1/步骤 S1/第一步 三种风格通用，但同一文档风格统一）？
- [ ] **规范 13**：流程图是否高亮/加粗重点步骤？（§7.4.1）+ 末尾是否给撰写点评/保护点定位提示？

任一项不达标 → 直接在 Step 8 改稿，不只报告。
